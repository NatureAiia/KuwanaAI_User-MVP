 ...
 */
function TypewriterBase(props: Props) {
    ...
    const easeTiming = ease as { duration?: number; delay?: number } | undefined
    const typeDelayMs = Math.max(0, (easeTiming?.duration ?? 0.07) * 1000)
    const holdMs = Math.max(0, (easeTiming?.delay ?? 1.5) * 1000)
    ...
    // index into a string that no longer exists. Adjusted during render
    ...
    const typeface = (font ?? {}) as Record<string, unknown>
...
export default function Typewriter(props: Record<string, unknown>) {
  return <TypewriterBase {...(__originkitPresetProps as Record<string, unknown>)} {...props} />;
}
