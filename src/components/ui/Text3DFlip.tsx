...
type Text3DFlipProps = {
  ...
  /** Repeats the flip on this interval (ms) — a third trigger alongside
   * hover/enter, for "play this automatically, on a loop" usage. */
  intervalMs?: number;
...
function Text3DFlipBase(props: Text3DFlipProps) {
  const {
    text = "STAGGER FLIP",
    font = DEFAULT_FONT,
    color = "#FFFFFF",
  ...
  // Automatic repeat trigger — the source component only supports
  // hover/enter; this is additive for "flip on a loop" usage (header idle
  // flip every 45s, loading-screen flip every 8s).
  useEffect(() => {
...
export default function Text3DFlip(props: Record<string, unknown>) {
  return <Text3DFlipBase {...(__originkitPresetProps as Record<string, unknown>)} {...props} />;
}
