...
import { FlagForInvestigationButton } from "@/components/admin/FlagForInvestigationButton";
...
export default async function AdminInvestigationsPage({
  ...
  return (
    <div className="mx-auto max-w-[900px] px-5 py-8 md:px-10">
      <h1 className="font-display text-[24px] font-bold">Investigations</h1>
      <p className="mt-1 text-[13px] text-text-secondary">
        Open and track enforcement cases against any provider&apos;s listings, market-wide — the same case queue the
        regulator portal uses, viewed and actioned from admin.
      </p>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        {LIVE_SECTORS.map((slug) => (
          <Link
            key={slug}
            href={`/admin/investigations?sector=${slug}`}
            className={`tap-target rounded-full border px-4 py-2 text-[13px] font-medium ${
      ...
      <section className="mt-6">
        <div className="flex items-center justify-between gap-3">
          ...
          {investigations.length > 0 && (
            <ExportCsvButton
              filename={`kuwana-admin-cases${activeSector ? `-${activeSector}` : ""}.csv`}
