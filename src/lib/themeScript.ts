 ...
 *
 * Light is the default: the class is only *added* when the stored preference
 * is explicitly "dark". Anything else — no preference, unreadable storage —
 * leaves the document light.
 *
 ...
 * hash authorises these exact bytes and nothing else. It also needs no
 * per-request state, which is what lets the root layout stay static instead
 * of every route being forced to dynamic rendering to read a nonce.
 */
 ...
 */
export const THEME_INIT_SCRIPT_CSP_HASH = "'sha256-yN2GRgxwo3otFtgm3v8xo1HfdOjkQZHkULuDTGww60A='";
