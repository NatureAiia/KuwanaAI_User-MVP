...
// Below this many published listings, a "median" is really just one or two
// competitors' numbers dressed up as a market average — not worth badging.
const MIN_LISTINGS_FOR_MEDIAN = 3;

/**
 * Per-attribute medians across every published listing in a category, used
 * to badge a listing's specs as below/at/above the market (see
 * describeMarketPosition in attributeDirection.ts). Returns {} for a
...
export const getCategoryAttributeMedians = cache(
  ttlCache(async (categoryId: string): Promise<Record<string, number>> => {
    ...
    if (listings.length < MIN_LISTINGS_FOR_MEDIAN) return {};
